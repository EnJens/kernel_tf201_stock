/*
 * arch/arm/mach-tegra/board-tf201-misc.c
 *
...
unsigned char tf201_chipid[17];
EXPORT_SYMBOL(tf201_chipid);

//PCBID is composed of nine GPIO pins predefined by HW schematic of Tegra3-series
unsigned int tf201_pcbid;
...
unsigned int tegra3_query_touch_module_pcbid(void)
{
	...
	touch_pcbid = (HW_DRF_VAL(TEGRA3_DEVKIT, MISC_HW, TOUCHL, tf201_pcbid)) +
		(HW_DRF_VAL(TEGRA3_DEVKIT, MISC_HW, TOUCHH, tf201_pcbid) << 1);
...
unsigned int tegra3_query_wifi_module_pcbid(void)
{
	...
	wifi_pcbid = HW_DRF_VAL(TEGRA3_DEVKIT, MISC_HW, WIFI, tf201_pcbid);
	ret = wifi_pcbid;
...
static ssize_t tf201_chipid_show(struct kobject *kobj,
...
static ssize_t tf201_backup_chipid_show(struct kobject *kobj,
	struct kobj_attribute *attr, char *buf)
{
	char *s = buf;
	unsigned long reg;
...
static ssize_t tf201_pcbid_show(struct kobject *kobj,
	struct kobj_attribute *attr, char *buf)
{
	char *s = buf;
	int i;
	...
	for (i = TEGRA3_DEVKIT_MISC_PCBID_NUM; i > 0; i--) {
		s += sprintf(s, "%c", tf201_pcbid & (1 << (i - 1)) ? '1' : '0');
	}
	s += sprintf(s, "b\n");
	return (s - buf);
}

static ssize_t tf201_projectid_show(struct kobject *kobj,
        struct kobj_attribute *attr, char *buf)
{
        char *s = buf;
...
static ssize_t tf201_projectname_show(struct kobject *kobj,
        struct kobj_attribute *attr, char *buf)
{
        char *s = buf;
...
static ssize_t tf201_vibctl_store(struct kobject *kobj,
...
static struct attribute *attr_list[] = {
	&tf201_chipid_attr.attr,
	&tf201_backup_chipid_attr.attr,
	&tf201_pcbid_attr.attr,
	&tf201_projectid_attr.attr,
	&tf201_projectname_attr.attr,
	&tf201_vibctl_attr.attr,
	NULL,
};
...
static struct platform_device *tf201_misc_device;
...
static int pcbid_init(void)
{
	...
	tf201_pcbid = (gpio_get_value(TEGRA_GPIO_PK3) << 8) |
...
int __init tf201_misc_init(void)
{
	...
	ret = platform_device_add(tf201_misc_device);
	if (ret) {
		pr_err("[MISC]: cannot add device to platform.\n");
		goto fail_platform_add_device;
	}

	ret = sysfs_create_group(&tf201_misc_device->dev.kobj, &attr_group);
	if (ret) {
...
fail_sysfs:
	platform_device_del(tf201_misc_device);

fail_platform_add_device:
	platform_device_put(tf201_misc_device);
